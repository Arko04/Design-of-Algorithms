...
void dfs(int node, int target, vector<pair<int, int>> adj[], vector<bool> &visited, set<int> &weights, vector<Edge> &path, map<pair<int, int>, int> &edge_index_map)
{
    if (node == target)
        return;
    ...
    for (auto &[next, weight] : adj[node])
    {
        if (!visited[next])
        {
            // Find the index of the edge (node, next) in the original edges list
            pair<int, int> edge_pair = (node < next) ? make_pair(node, next) : make_pair(next, node);
            int edge_index = edge_index_map[edge_pair];

            // Create a new edge with the correct index
            Edge edge = {node, next, weight, edge_index};
            path.push_back(edge);

            // Perform DFS
            dfs(next, target, adj, visited, weights, path, edge_index_map);
...
int main()
{
    ...
    // Create a map to find the index of an edge (u, v)
    map<pair<int, int>, int> edge_index_map;
    for (const auto &edge : edges)
    {
        pair<int, int> edge_pair = (edge.u < edge.v) ? make_pair(edge.u, edge.v) : make_pair(edge.v, edge.u);
        edge_index_map[edge_pair] = edge.index;
    }
    ...
    for (const auto &edge : edges)
    {
        ...
        set<int> weights;
        vector<Edge> path;
        dfs(edge.u, edge.v, adj, visited, weights, path, edge_index_map);
