    ...
    long long root_index) 
{
    ...
    nodes[max_e_leaf].max_e_leaf->e = max(heights[max_e_leaf],nodes[root_index].e);
}
    ...
    long long current)
{
    ...
    max_leaf_node->e = max(max_leaf_node->e, nodes[current].height);
    nodes[current].max_e_leaf = max_leaf_node;
    nodes[current].max_e_leaf->e = max(max_leaf_node->e, nodes[current].height);
...
int main()
{
    ...
    // Prepare a visited array and propagate `e` values
    vector<bool> visited(n + 1, false);
    propagateE(adj_list, nodes, root);

    // Create a heights vector
    vector<long long> heights(n + 1, 0);

    // Update leaf set and height
    updateLeafSetAndHeight(nodes, adj_list, heights, visited, root);
