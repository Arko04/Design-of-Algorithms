...
struct Node
{
    ...
    Node(long long i, long long h) : index(i), height(h), max_e_leaf(nullptr) {}
...
int main()
{
    ...
    // Debug: Print e values and max_e_leaf of each node
    cout << "Node e values and max_e_leaf:" << endl;
    for (long long i = 1; i <= n; ++i)
    {
        cout << "Node " << i << ": ";
        if (nodes[i].children.empty() && adj_list[i].size() == 1)
        {
            cout << "e = " << nodes[i].e << ", max_e_leaf = itself (leaf)" << endl;
        }
        else if (nodes[i].max_e_leaf != nullptr)
        {
            cout << "max_e_leaf = Node " << nodes[i].max_e_leaf->index
                 << " (height = " << nodes[i].max_e_leaf->height << ")" << endl;
        }
    }

    // updateLeafSetAndHeight(
    // nodes, 
    // adj_list, 
    // heights, 
    // visited, 
    ...
    // cout << long long calculateLeafESum(const vector<Node> &nodes) << end;
